    ...
    public function accept_offer(){
        ...
        if($offer_type=='refuse'){
            ...
            if($before_amount<$offer_amount){
                ...
                DB::table('withdraw_paypal')->insert($insert_paypal_data);
                DB::table('withdraw_to_admin')->where('_id',$offer_id)->update(array('status'=>'accept',));
            }
        }
        $data = DB :: table('withdraw_to_admin')->join('users','users.id','=','withdraw_to_admin.user_id')->get();
        if($error=='')
